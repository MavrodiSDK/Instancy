...
namespace il2cpp {
	namespace methods {
		...
		static auto intialize_method = rb::pattern::find(cry("GameAssembly.dll"), cry("48 83 EC 48 48 8B 05 ? ? ? ? 48 63 90 ? ? ? ?"));
	...
	void init() {
		methods::intialize_method = rb::pattern::find(cry("GameAssembly.dll"), cry("48 83 EC 48 48 8B 05 ? ? ? ? 48 63 90 ? ? ? ?"));
	...
	uintptr_t init_class(const char* name, const char* name_space = cry("")) {
	...
	uintptr_t method(const char* kl, const char* name, int argument_number = -1, char* arg_name = cry(""), const char* name_space = cry(""), int selected_argument = -1) {
		uintptr_t iter = 0;
		uintptr_t f;
		...
		while (f = methods::class_get_methods(klass, &iter)) {
			...
			if (m_strcmp(st, (char*)name)) {
				if (selected_argument >= 0 && arg_name) {
					uintptr_t args = safe_read(f + 0x28, uintptr_t);
	...
	uintptr_t hook(void* our_func, const char* function_to_hook, const char* class_to_hook, const char* name_space = cry(""), int argument_number = -1, char* argument_name = cry("")) {
...
uintptr_t hook_virtual_function(const char* classname, const char* function_to_hook, void* our_func, const char* name_space = cry("")) {
	uintptr_t search = *reinterpret_cast<uintptr_t*>(il2cpp::method(classname, function_to_hook, -1, cry(""), name_space));
